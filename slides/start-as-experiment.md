...
* InnerSourceを検討しています、その目的はソフトウェアプロジェクトのコラボレーションの効率化です
* ほとんどの管理者はオープンソースの作業モデルに精通していなく、階層的なトップダウンの制御スタイルの管理に慣れています。
* InnerSourceのアイデアは、社内の開発者に非常に人気があり、多くの開発者がオープンソースソフトウェアを使用しているか積極的に開発しているためです.
...
* マネージャーはインナーソースの効果が期待できるかを検証する必要があり、改善があるかを測定することを含みます
* インナーソースイニシアチブが開発者の間で大きな支持を得る可能性が高い場合、シャットダウンには不人気であり、実施は困難です
* インナーソーススタイルの実装は根本的な逸脱であり、既存のプロセスは適用されず管理プロセスが失われる可能性があり、規制や法的規制のない場所で活動をする可能性があります
